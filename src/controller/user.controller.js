...
export default class UserController {
	...
	static async updateUser(data) {
		...
		return await fetch(this.base_url, {
			...
			body: JSON.stringify({usr_name: data.usr_name, usr_image: data.usr_image}),
		})
		.then((res) => res.json())
		.then((resp) => {
	...
	static getLoggedUserIformation() {
		let storedUser = JSON.parse(localStorage.getItem("@kenzie-habits:user"));
		if (storedUser == null) {
			return null;
		}
		return new User(storedUser.usr_name, storedUser.usr_email, storedUser.usr_image);
	}
